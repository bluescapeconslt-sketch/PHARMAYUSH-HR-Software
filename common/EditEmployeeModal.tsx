    ...
    const fetchDropdownData = async () => {
        if (isOpen && employee) {
            ...
            try {
                ...
                console.error("Failed to load data for Edit Employee modal", error);
    ...
    <Modal isOpen={isOpen} onClose={handleClose} title="Edit Employee Details">
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex justify-end gap-4 pt-4">
          ...
          <button type="submit" disabled={isLoading} className="px-4 py-2 text-sm font-medium text-white bg-indigo-600 rounded-md disabled:bg-indigo-300">Save Changes</button>
        </div>
      </form>
    </Modal>
  );
