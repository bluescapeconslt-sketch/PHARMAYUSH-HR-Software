...
import BirthdayNotification from './components/common/BirthdayNotification.tsx';
import DailyMotivation from './components/common/DailyMotivation.tsx';
import NoticeBoard from './components/common/NoticeBoard.tsx';
    ...
    return (
        <Card title="Time Clock">
            <div className="flex flex-col items-center justify-center gap-4 text-center">
                <div className="text-4xl sm:text-5xl font-bold text-gray-800 dark:text-gray-100 tracking-wider">
                ...
                <div className="min-h-[72px] mt-4 flex flex-col justify-center items-center">
                    {isLoading ? (
                        <div className="h-6 bg-slate-200 dark:bg-gray-700 rounded w-48 animate-pulse"></div>
                    ) : status === 'out' ? (
                        <p className="text-lg text-gray-600 dark:text-gray-300">You are <span className="font-bold text-red-600">Punched Out</span>.</p>
                    ) : (
                         <div className="text-center">
                            <p className="text-lg text-gray-600 dark:text-gray-300">You are <span className="font-bold text-green-600">Punched In</span>.</p>
                            <p className="text-2xl font-mono font-bold text-gray-800 dark:text-gray-100 mt-2">{workDuration || '00:00:00'}</p>
                            <p className="text-xs text-gray-500 dark:text-gray-400">Current Session Duration</p>
                            {showUndo && (
                                <div className="mt-2">
...
const PharmayushBuddy: React.FC = () => {
    ...
    return (
        <Card>
            <h3 className="text-xl font-semibold text-gray-700 dark:text-gray-200 mb-4 text-center">Pharmayush Buddy's Wellness Tip</h3>
                ...
                <div className="flex-grow min-h-[60px]">
                    {isLoading ? (
                        <div className="space-y-2 pt-2">
                           ...
                           <div className="h-4 bg-slate-200 dark:bg-gray-700 rounded w-3/4 animate-pulse mx-auto"></div>
                       </div>
                    ) : (
                        <p className="text-gray-700 dark:text-gray-300 text-base italic">"{tip}"</p>
                    )}
                </div>
...
const TodaysMeetings: React.FC = () => {
    ...
    if (isLoading) {
        return <Card title="Today's Meetings"><div className="text-center p-4 text-gray-500 dark:text-gray-400">Loading meetings...</div></Card>;
    }
        ...
        return (
            <Card title="Today's Meetings">
                <div className="flex flex-col items-center justify-center h-full text-center">
                    ...
                    <p className="text-gray-500 dark:text-gray-400 text-sm">No meetings scheduled. Enjoy the focus time!</p>
                </div>
            </Card>
        );
    }
    ...
    return (
        <Card title="Today's Meetings">
            <div className="space-y-3 max-h-48 overflow-y-auto">
                {meetings.map(m => (
                    <div key={m.id} className="flex items-center gap-3 p-2 rounded-md bg-gray-50 dark:bg-gray-700/50">
                        <div className="font-semibold text-indigo-600 dark:text-indigo-400">{m.time}</div>
                        <div>
                            <p className="text-sm font-medium text-gray-800 dark:text-gray-200">{m.title}</p>
                            <p className="text-xs text-gray-500 dark:text-gray-400">{m.departmentName}</p>
                        </div>
                    </div>
                ))}
            </div>
    ...
    return (
        <div>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
                <Card className="bg-gradient-to-r from-indigo-50 via-purple-50 to-pink-50 dark:from-gray-700 dark:via-indigo-900/50 dark:to-gray-800">
                    <DailyMotivation />
                </Card>
                <PharmayushBuddy />
            </div>
                ...
                {canViewAllEmployees && (
                    <>
                        <Card title="Total Employees">
                            {isLoading ? <div className="h-10 bg-slate-200 dark:bg-gray-700 rounded w-1/2 animate-pulse"></div> : <p className="text-4xl font-bold text-gray-800 dark:text-gray-100">{employees.length}</p>}
                            <p className="text-sm text-gray-500 dark:text-gray-400">{activeEmployees.length} Active</p>
                        </Card>
                    </>
                )}
                <Card title={requestsCardTitle}>
                    {isLoading ? <div className="h-10 bg-slate-200 dark:bg-gray-700 rounded w-1/2 animate-pulse"></div> : <p className="text-4xl font-bold text-indigo-600 dark:text-indigo-400">{pendingRequests.length}</p>}
                    <p className="text-sm text-gray-500 dark:text-gray-400">Awaiting approval</p>
                </Card>
                {canViewAllEmployees && (
                    <>
                        <Card title="Employees on Leave">
                             {isLoading ? <div className="h-10 bg-slate-200 dark:bg-gray-700 rounded w-1/2 animate-pulse"></div> : <p className="text-4xl font-bold text-gray-800 dark:text-gray-100">{onLeaveEmployeesCount}</p>}
                            <p className="text-sm text-gray-500 dark:text-gray-400">Currently on leave</p>
                        </Card>
                        <Card title="Upcoming Birthdays">
                            {isLoading ? <div className="h-10 bg-slate-200 dark:bg-gray-700 rounded w-1/2 animate-pulse"></div> : <p className="text-4xl font-bold text-gray-800 dark:text-gray-100">{upcomingBirthdays.length}</p>}
                            <p className="text-sm text-gray-500 dark:text-gray-400">In the next 7 days</p>
                        </Card>
                    </>
                )}
            </div>
                                    ...
                                    <tr key={req.id}>
                                        <td className="px-4 py-2 whitespace-nowrap">
                                            <div className="flex items-center">
                                                ...
                                                <span className="ml-2 text-sm font-medium text-gray-900 dark:text-gray-200">{req.employeeName}</span>
                                            </div>
                                        </td>
                                        <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{req.startDate} to {req.endDate}</td>
                                        <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{req.leaveType}</td>
                                    </tr>
                                ))}
                                {pendingRequests.length === 0 && (
                                  <tr>
                                    <td colSpan={3} className="text-center text-gray-500 dark:text-gray-400 py-4">No pending requests.</td>
                                  </tr>
                                )}
                            </tbody>
                        </table>
